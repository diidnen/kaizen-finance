...
public class Service {
    ...
    @GetMapping("/generate-quote-reference")
    public ResponseEntity<?> generateQuoteReference() {
        try {
            // 获取当前最大的 quote reference 序号
            // 查找所有订单中最大的序号
            List<OrderData> allOrders = orderRepository.findAll();
            long maxSequence = 0;
            
            for (OrderData order : allOrders) {
                if (order.getQuoteReference() != null && order.getQuoteReference().startsWith("QUOTE-")) {
                    try {
                        String seqStr = order.getQuoteReference().substring(6); // 去掉 "QUOTE-" 前缀
            ...
            // 生成顺序编号：QUOTE-000001, QUOTE-000002, ...
            String quoteRef = String.format("QUOTE-%06d", maxSequence + 1);
            
            Map<String, Object> response = new HashMap<>();
            response.put("code", 200);
            response.put("quoteReference", quoteRef);
            ...
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("code", 500);
            errorResponse.put("message", "Failed to generate quote reference: " + e.getMessage());
    ...
    public ResponseEntity<?> submitOrder(@RequestBody Map<String, Object> request) {
        try {
            ...
            String quoteReference = (String) request.get("quoteReference");
            ...
            for (Map<String, Object> service : services) {
                ...
                order.setQuoteReference(quoteReference);
                order.setServiceName((String) service.get("name"));
                order.setServiceType((String) service.get("type"));
                // 获取价格并进行格式化
                ...
                // 设置时间信息
                order.setStartTime((String) service.get("startTime"));
                order.setEndTime((String) service.get("endTime"));
                order.setEstimateTime((String) service.get("estimateTime"));
